...
export default function ArtifactsView() {
  ...
  return (
    <div className="flex h-full flex-col bg-[#0A0A0A]">
      <div className="border-b border-zinc-900 px-4 md:px-6 py-5 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white flex items-center gap-2">
            Artifacts <Sparkles size={18} className="text-violet-500" />
          </h2>
          <p className="text-sm text-zinc-400 mt-1 hidden sm:block">AI-generated reports, exports, and final outputs.</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        <div className="space-y-4">
          {artifacts.map(artifact => (
            <div key={artifact.id} className="group relative flex flex-col sm:flex-row sm:items-center gap-4 rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5 transition hover:border-zinc-700 hover:bg-zinc-800/80">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-zinc-950 border border-zinc-800 shadow-inner">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-zinc-400 line-clamp-1">{artifact.description}</p>
              ...
              <div className="flex flex-row items-center gap-2 mt-4 sm:mt-0 w-full sm:w-auto">
                <button className="flex flex-1 sm:flex-none justify-center items-center gap-2 rounded-lg bg-zinc-800 px-3 py-2 text-sm font-medium text-zinc-300 transition hover:bg-zinc-700 hover:text-white">
                  <ExternalLink size={16} />
                  View
                </button>
                <button className="flex flex-1 sm:flex-none justify-center items-center gap-2 rounded-lg bg-zinc-100 px-3 py-2 text-sm font-medium text-zinc-900 transition hover:bg-white">
                  <Download size={16} />
                  Download
                </button>
              </div>
        ...
        {artifacts.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-medium text-zinc-300">No artifacts generated yet</h3>
            <p className="mt-2 text-sm text-zinc-500 max-w-sm">Interact with the AI planner to generate reports, plans, and exports.</p>
          </div>
        )}
      </div>
    </div>
